...
import {getFormStud} from "../../http/DIdgitalSystemApi";
...
const ZayavlenyaPoFormamObychenyaDidgital = ({Inst}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getFormStud(Inst);
        console.log(data)
        setFetchedData(data);
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if(fetchedData) {
      ...
      chartRef.current = new Chart(ctx, {
        type: 'bar',
        data: {
          labels: ['Очная', 'Очно-заочная', 'Заочная'],
          datasets: [{
...
export default ZayavlenyaPoFormamObychenyaDidgital;
