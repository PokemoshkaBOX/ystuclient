...
import {getAvgBall} from "../../http/DIdgitalSystemApi";
...
const StatPoBallDidgital = ({Inst}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getAvgBall(Inst);
        console.log(data)
        setFetchedData(data);
      } catch (error) {
    ...
    if(fetchedData) {
      ...
      chartRef.current = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              label: 'Распределение среднего балла ЕГЭ',
              data: counts,
              borderWidth: 1,
            },
              {
              label: 'Распределение среднего балла ЕГЭ по согласиям',
              data: counts1,
              borderWidth: 1,
            },
...
export default StatPoBallDidgital;
